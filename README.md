...
Читательский отчёт на русском с раскрываемыми карточками можно скачать кнопкой
**Скачать отчёт с подробностями**. Каждый из пяти разделов новости открывается независимо; можно раскрыть несколько сразу. Это самостоятельный HTML-файл: откройте его в браузере,
подключение к API для просмотра не нужно. Сразу видны суть, значимость и достоверность;
внутри каждой новости — «Что произошло», «Почему важно», «Как применить», «Ограничения»
и «Источники». Подробные доказательства и расход токенов раскрываются отдельно.

```sh
python -m app.run --html data/report.html --output data/report.md --json data/report.json
python -m app.run --from-json data/report.json --html data/report.html --output data/report.md
```

Вторая команда только переоформляет сохранённые данные, без нового поиска и расходов на модель.
Markdown сохраняет ту же структуру; раскрытие `<details>` зависит от программы просмотра,
поэтому для чтения рекомендуется HTML. Названия продуктов, принятые технические термины,
исходные цитаты и машиночитаемые поля JSON сохраняются. Интерфейс карточек приложения не менялся.
...
Проверка текущих данных 25 сентября 2026 прошла весь pipeline: 59 результатов поиска,
8 кандидатов, 6 проверены в ограниченном тесте, 1 подтверждённое событие и 2 неподтверждённых.
Подтверждён запуск плагина Amazon Selling Partner для Quick/Claude по первичному анонсу
и независимым публикациям. [Сводка проверки](evaluation/current_news_acceptance_2026-09-25.json)
содержит окно, ссылки и usage; это проверка работоспособности, а не измерение полноты рынка.
