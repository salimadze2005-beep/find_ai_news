...
from app.agents.verifier import evidence_errors, unsupported_numbers
...
def analyze_context(event, llm, search, pages, end, trace):
    ...
    def validate(context):
        ...
        missing = unsupported_numbers([context.what_changed, context.hype_reason] + context.limitations, baseline)
        if missing:
            raise ProviderError("Context introduces a number absent from cited evidence: " + missing)
