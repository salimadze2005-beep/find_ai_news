...
def analyze_impact(event, context, context_sources, llm, settings, trace):
    ...
    def validate(impact):
        missing = unsupported_numbers([impact.what_happened, impact.why_it_matters, impact.product_impact, impact.what_to_try,
                           impact.alfa_bank_relevance] + impact.practical_opportunities + impact.inference_limitations, baseline)
        if missing:
            raise ProviderError("Impact introduces a number absent from cited evidence: " + missing)
